...
def flow_sizes_constants(rgb_shape: Shape) -> Dict:
    flow_sizes = Dict()
    if rgb_shape.height == 480:
...
def restore_flow_original_size(flow: np.ndarray, flow_sizes: Dict) -> np.ndarray:
...
def add_missing_borders(flow: np.ndarray, flow_sizes: Dict) -> np.ndarray:
    return cv2.copyMakeBorder(flow,
...
def resize_to_flow_shape_and_remove_borders(img: np.ndarray, flow_sizes: Dict) -> np.ndarray:
...
def caculate_inertia_matrix_components(flow: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    u = np.power(flow[:, :, 0], 2)
    v = np.power(flow[:, :, 1], 2)
    uv = flow[:, :, 0] * flow[:, :, 1]
    return u, v, uv
...
def build_second_moment_matrix(components: tuple[np.ndarray, np.ndarray, np.ndarray], mask=None, indices=None) -> np.ndarray:
...
def caculate_dominant_orientation_vector(M: np.ndarray, gradient_vectors: np.ndarray) -> np.ndarray:
